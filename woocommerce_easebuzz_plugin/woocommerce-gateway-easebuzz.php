                        ...
                        if($order_id != ''){
                            try{
                                ...
                                if( $order->status !=='completed'){
                                    if($hash == $checkhash){ 
                                try{
                                    $table_name = $wpdb->prefix . 'wc_transaction_details';     
                                    $wpdb->insert(
                                        $table_name,array(
                                            'order_id' => $_REQUEST['udf1'],
                                            'merchant_txn_id' => $_REQUEST['txnid'],
                                            'easebuzz_txn_id' => $_REQUEST['easepayid'],
                                            'bank_ref_number' => $_REQUEST['bank_ref_num'],
                                            'transaction_date'=> $_REQUEST['addedon'],
                                            'total_amount' => $_REQUEST['amount'],
                                            'transaction_status' => $_REQUEST['status'],
                                            'deduction_percentage' => $_REQUEST['deduction_percentage'],
                                            'service_charge' => $_REQUEST['service_charge'],
                                            'service_tax' => $_REQUEST['service_tax'],
                                            'net_debit_amount' => $_REQUEST['net_amount_debit'],
                                            'payment_mode' => $_REQUEST['mode'],
                                            'cashback_percentage' => $_REQUEST['cash_back_percentage'],
                                            'product_info' => $_REQUEST['productinfo'],
                                            'payment_source_name' => $_REQUEST['payment_source'],
                                            'pg_type' => $_REQUEST['PG_TYPE'],
                                            'surl' => $_REQUEST['surl'],
                                            'furl' => $_REQUEST['furl'],
                                            'payment_error' => $_REQUEST['error'],
                                            ...
                                            'udf8' => $_REQUEST['udf8'],
                                            'udf9' => $_REQUEST['udf9'],
                                            'udf10' => $_REQUEST['udf10'],
                                            'udf11' => "",
                                            'udf12' => "",
                                            'udf13' => "",
                                            'udf14' => "",
                                            'udf15' => $this->settings['test_mode'],
                                            'created_date' => $_REQUEST['addedon'],
                                            'updated_date' => date('Y-m-d H:i:s'),
                                        ),
                                    array('%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s','%s')
                                    );
                                }catch (Exception $e){
    ...
    if( isset($response['txnid']) && isset($response['easepayid']) ){
        ...
        if($order_id != ''){
            try{
                ...
                // $table_name = $wpdb->prefix . 'wc_transaction_details';
                $wpdb->insert(
                    'wp_wc_transaction_details',
                    array(
                            'order_id' => $response['udf1'],
                            'merchant_txn_id' => $response['txnid'],
                            'easebuzz_txn_id' => $response['easepayid'],
                            'bank_ref_number' => $response['bank_ref_num'],
                            'transaction_date'=> $response['addedon'],
                            'total_amount' => $response['amount'],
                            'transaction_status' => $response['status'],
                            'deduction_percentage' => $response['deduction_percentage'],
                            'service_charge' => $response['service_charge'],
                            'service_tax' => $response['service_tax'],
                            'net_debit_amount' => $response['net_amount_debit'],
                            'payment_mode' => $response['mode'],
                            'cashback_percentage' => $response['cash_back_percentage'],
                            'product_info' => $response['productinfo'],
                            'payment_source_name' => $response['payment_source'],
                ...
                if( $order->status !=='completed'){
                    if($hash == $checkhash){
                        ...
                        if($status=="success"){
                            ...
                            //$return_response['message'] = "<>Oops, Sorry your transaction has been failed. Please try again<br><br>";
                            //**************************************************************************************************// 
