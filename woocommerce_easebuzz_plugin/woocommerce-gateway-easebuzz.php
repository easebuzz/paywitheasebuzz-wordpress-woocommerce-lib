...
$payment_request_parameter = array(
    ...
    'furl' => trim($redirect_fail_url),
                    ...
                    if($response["status"]===1){
                        ...
                        jQuery(function(){
                            jQuery("body").block({
                                    message: "'.__('Oops, Server error, Please try again. \n'.$response["data"], 'woo_payeasebuzz').'",
                                    overlayCSS: {
                                        background	: "#fff",
                                        opacity	: 0.8
                                        },
                                    ...
                                    if($hash == $checkhash){ 
                                        ...
                                        if($status=="success"){
                                            ...
                                            if($order->status == 'processing'){
                                                $order->add_order_note('Easebuzz payment successful.<br/>Easbeuzz ID: '.$_REQUEST['easepayid'].'<br/> Transaction ID:'.$_REQUEST['txnid'].' <br/>Bank Ref: '.$_REQUEST['bank_ref_num'].'('.$_REQUEST['mode'].')');
                                            ...
                                            $order->add_order_note('Easebuzz payment status is pending<br/>Easebuzz ID: '.$_REQUEST['easepayid'].' <br/> Transaction ID:'.$_REQUEST['txnid'].' <br/>Bank Ref: '.$_REQUEST['bank_ref_num'].'('.$_REQUEST['mode'].')');
                                            ...
                                            $trans_authorised = true;
                                            $this->msg['message'] = "Thank you for shopping with us. You have canceled the transaction. ";
                                            $this->msg['class'] = 'pending';
                                            $order->add_order_note('Easebuzz payment status is userCancelled<br/>Easebuzz ID: '.$_REQUEST['easepayid'].' <br/> Transaction ID:'.$_REQUEST['txnid'].' <br/>Bank Ref: '.$_REQUEST['bank_ref_num'].'('.$_REQUEST['mode'].')');
                                            ...
                                            $order->add_order_note('Transaction Status: '.$status.'<br/>Easebuzz ID: '.$_REQUEST['easepayid'].' <br/> Transaction ID:'.$_REQUEST['txnid'].' <br/>Bank Ref: '.$_REQUEST['bank_ref_num'].'('.$_REQUEST['mode'].')');
            ...
            try{
                ...
                if( $order->status !=='completed'){
                    if($hash == $checkhash){
                        $status = strtolower($status);
                        if($status=="success"){
                            $order->add_order_note('Easebuzz payment successful.<br/>Easbeuzz ID: '.$response['easepayid'].' <br/> Transaction ID:'.$response['txnid'].'<br/>Bank Ref: '.$response['bank_ref_num'].'('.$response['mode'].')');
                            $woocommerce->cart->empty_cart();
                            ...
                            $order->add_order_note('Easebuzz payment status is pending<br/>Easebuzz ID: '.$response['easepayid'].' <br/> Transaction ID:'.$response['txnid'].' <br/>Bank Ref: '.$response['bank_ref_num'].'('.$response['mode'].')');
                            $order->update_status('pending payment');
                            $woocommerce->cart->empty_cart();
                            ...
                            $order->add_order_note('Transaction ERROR: '.$response['error'].'<br/>Easebuzz ID: '.$response['easepayid'].' <br/> Transaction ID:'.$response['txnid'].' <br/>Bank Ref: '.$response['bank_ref_num'].'('.$response['mode'].')');
                        }
                    }else{
                        $return_response['class'] = 'error';
